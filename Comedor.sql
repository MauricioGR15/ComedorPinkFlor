...
CREATE TABLE PagoMenu(
	pago_id int primary key,
	menu_id int not null foreign key references Menus(menu_id),
	RFC nvarchar(13) not null foreign key references Tutores(RFC),
	matricula int not null foreign key references Alumnos(matricula),
	total money not null
)

CREATE TABLE PagoConcepto(
	pago_id int foreign key references PagoMenu(pago_id),
	folio int identity,
